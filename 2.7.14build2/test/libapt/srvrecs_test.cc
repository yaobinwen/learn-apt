...
TEST(SrvRecTest, Randomness)
{
  ...
  EXPECT_TRUE(std::all_of(first_pull.begin(), first_pull.end(), [](SrvRec const &R)
                          { return R.priority == 20; }));
  EXPECT_TRUE(std::all_of(second_pull.begin(), second_pull.end(), [](SrvRec const &R)
                          { return R.priority == 20; }));
  ...
  EXPECT_TRUE(std::all_of(base2.begin(), base2.end(), [](SrvRec const &R)
                          { return R.priority == 30; }));
}
