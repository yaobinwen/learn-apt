...
bool PackageMap::GenPackages(Configuration &Setup, struct CacheDB::Stats &Stats)
{
  ...
  c0out << Packages.Stats.Packages << " files " <<
     ...
     SizeToStr(Packages.Stats.Bytes) << "B " << TimeToStr(GetTimeDeltaSince(StartTime)) << endl;
...
bool PackageMap::GenSources(Configuration &Setup, struct CacheDB::Stats &Stats)
{
  ...
  c0out << Sources.Stats.Packages << " pkgs in " << TimeToStr(GetTimeDeltaSince(StartTime)) << endl;
                             ...
                             unsigned long &Left)
{
  ...
  c0out << Contents.Stats.Packages << " files " << SizeToStr(Contents.Stats.Bytes) << "B " << TimeToStr(GetTimeDeltaSince(StartTime)) << endl;
...
{
  std::cout << _("Usage: apt-ftparchive [options] command\n"
...
static bool Clean(CommandLine &CmdL)
{
  ...
  for (vector<PackageMap>::iterator I = PkgList.begin(); I != PkgList.end();)
  {
    ...
    I = std::find_if(I, PkgList.end(),
                     [&](PackageMap const &PM)
                     {
                       return PM.BinCacheDB != I->BinCacheDB || PM.SrcCacheDB != I->SrcCacheDB;
                     });
  }
