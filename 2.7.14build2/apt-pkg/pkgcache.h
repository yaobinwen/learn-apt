...
template <typename T>
inline T *operator+(T *p, map_pointer<T> m) { return p + uint32_t(m); }
template <typename T>
inline bool operator==(map_pointer<T> u, map_pointer<T> m) { return uint32_t(u) == uint32_t(m); }
template <typename T>
inline bool operator!=(map_pointer<T> u, map_pointer<T> m) { return uint32_t(u) != uint32_t(m); }
template <typename T>
inline bool operator<(map_pointer<T> u, map_pointer<T> m) { return uint32_t(u) < uint32_t(m); }
...
inline bool operator>(map_pointer<T> u, map_pointer<T> m) { return uint32_t(u) > uint32_t(m); }
template <typename T>
inline uint32_t operator-(map_pointer<T> u, map_pointer<T> m) { return uint32_t(u) - uint32_t(m); }
template <typename T>
bool operator==(map_pointer<T> m, std::nullptr_t) { return uint32_t(m) == 0; }
template <typename T>
bool operator!=(map_pointer<T> m, std::nullptr_t) { return uint32_t(m) != 0; }
...
struct pkgCache::Version
{
  ...
  enum VerMultiArch
  {
    No = 0,             /*!< is the default and doesn't trigger special behaviour */
