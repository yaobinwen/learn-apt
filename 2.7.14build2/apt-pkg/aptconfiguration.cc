...
std::vector<std::string> const Configuration::getCompressionTypes(bool const &Cached)
{
       ...
       o != order.end(); ++o)
  {
    ...
    if (std::find_if(compressors.begin(), compressors.end(), [&app](APT::Configuration::Compressor const &c)
                     { return c.Name == app; }) == compressors.end())
      continue;
    types.push_back(*o);
  }
  ...
  for (; Types != 0; Types = Types->Next)
  {
    ...
    if (std::find_if(compressors.begin(), compressors.end(), [&Types](APT::Configuration::Compressor const &c)
                     { return c.Name == Types->Value; }) == compressors.end())
      continue;
...
std::vector<std::string> const Configuration::getArchitectures(bool const &Cached)
{
  ...
  auto newend = std::remove_if(archs.begin(), archs.end(), [](auto const &a)
                               { return a.empty(); });
...
std::vector<APT::Configuration::Compressor> const Configuration::getCompressors(bool const Cached)
{
...
#define APT_ADD_COMPRESSOR(NAME, EXT, BINARY, ARG, DEARG, COST)    \
  {                                                                \
    CompressorsDone.push_back(NAME);                               \
    compressors.emplace_back(NAME, EXT, BINARY, ARG, DEARG, COST); \
  }
