                                    ...
                                    pkgRecords *Recs)
{
  ...
  bool const ordering =
     _config->FindB("PackageManager::UnpackAll", true) ? List->OrderUnpack() : List->OrderCritical();
  if (ordering == false)
                                        ...
                                        bool *const Bad, bool *const Changed)
{
  ...
  switch (action)
  {
  case UNPACK_IMMEDIATE:
    success = SmartUnPack(DepPkg, true, Depth + 1);
    break;
  case UNPACK:
    success = SmartUnPack(DepPkg, false, Depth + 1);
    break;
  case CONFIGURE:
    success = SmartConfigure(DepPkg, Depth + 1);
    break;
  }
  if (PkgLoop == false)
  ...
  if ((Cache[Pkg].InstVerIter(Cache)->MultiArch & pkgCache::Version::Same) == pkgCache::Version::Same &&
         ...
         P.end() == false; P = Pkg.Group().NextPkg(P))
    {
      if (Pkg == P || List->IsFlag(P, pkgOrderList::Configured) == true ||
          ...
          Cache[P].InstallVer == 0 || (P.CurrentVer() == Cache[P].InstallVer && (Cache[Pkg].iFlags & pkgDepCache::ReInstall) != pkgDepCache::ReInstall))
        continue;
...
pkgPackageManager::OrderResult pkgPackageManager::OrderInstall()
{
  ...
  bool const ordering =
     _config->FindB("PackageManager::UnpackAll", true) ? List->OrderUnpack(FileNames) : List->OrderCritical();
  if (ordering == false)
  {
  ...
  for (pkgOrderList::iterator I = List->begin(); I != List->end(); ++I)
  {
    ...
    else if (SmartUnPack(Pkg, List->IsFlag(Pkg, pkgOrderList::Immediate), 0) == false)
      return Failed;
    DoneSomething = true;
