...
PackageNameMatchesFnmatch::PackageNameMatchesFnmatch(std::string const &Pattern) : Pattern(Pattern) {}
...
PackageArchitectureMatchesSpecification::PackageArchitectureMatchesSpecification(std::string const &pattern, bool const pisPattern) : literal(pattern), complete(CompleteArch(pattern, pisPattern)), isPattern(pisPattern)
{
}
...
ANDMatcher &ANDMatcher::AND(Matcher *const matcher)
{
...
ORMatcher &ORMatcher::OR(Matcher *const matcher)
{
  matchers.push_back(matcher);
  return *this;
}
