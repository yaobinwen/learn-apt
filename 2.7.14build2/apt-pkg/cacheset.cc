                                 ...
                                 pkgCacheFile &Cache, std::string const &pattern)
{
  ...
  case UNKNOWN:
    return false;
  case REGEX:
    return PackageFromRegEx(pci, Cache, pattern);
  case TASK:
    return PackageFromTask(pci, Cache, pattern);
  case FNMATCH:
    return PackageFromFnmatch(pci, Cache, pattern);
  case PACKAGENAME:
    return PackageFromPackageName(pci, Cache, pattern);
  case STRING:
    return PackageFromString(pci, Cache, pattern);
  case PATTERN:
    return PackageFromPattern(pci, Cache, pattern);
  }
  return false;
}
/*}}}*/
                                           ...
                                           bool const onlyFromName)
{
       ...
       P != pkgset.end(); ++P)
  {
    ...
    {
      pkgVersionMatch Match(ver, (verIsRel == true ? pkgVersionMatch::Release : pkgVersionMatch::Version));
      V = Match.Find(P);
                                               ...
                                               CacheSetHelper &helper)
{
  ...
  case CacheSetHelper::CANDIDATE:
    ...
    if (((Cache[D] & pkgDepCache::DepCVer) == 0) != D.IsNegative())
      return found;
    return insertAllTargetVersions([&](pkgCache::PkgIterator const &OP)
                                   { return Cache[OP].CandidateVerIter(Cache); });
  case CacheSetHelper::INSTALLED:
    return insertAllTargetVersions([&](pkgCache::PkgIterator const &OP)
                                   { return OP.CurrentVer(); });
  case CacheSetHelper::CANDINST:
                                       ...
                                       std::string const &pattern)
{
  ...
  {
  case REGEX:
    canNotFindRegEx(pci, Cache, pattern);
    break;
  case TASK:
    canNotFindTask(pci, Cache, pattern);
    break;
  case FNMATCH:
    canNotFindFnmatch(pci, Cache, pattern);
    break;
  case PACKAGENAME:
    canNotFindPackage(pci, Cache, pattern);
    break;
  case STRING:
    canNotFindPackage(pci, Cache, pattern);
    break;
...
void CacheSetHelper::canNotFindVersion(enum VerSelector const select, VersionContainerInterface *const vci, pkgCacheFile &Cache, pkgCache::PkgIterator const &Pkg)
{
  ...
  {
  case ALL:
    canNotFindAllVer(vci, Cache, Pkg);
    break;
  case INSTCAND:
    canNotFindInstCandVer(vci, Cache, Pkg);
    break;
  case CANDINST:
    canNotFindCandInstVer(vci, Cache, Pkg);
    break;
  case NEWEST:
  ...
  case CANDIDATE:
    ...
    break;
  case INSTALLED:
    canNotFindInstalledVer(Cache, Pkg);
    break;
  case CANDANDINST:
    canNotGetCandInstVer(Cache, Pkg);
    break;
  case RELEASE:
    canNotGetVerFromRelease(Cache, Pkg, getLastVersionMatcher());
    break;
  case VERSIONNUMBER:
...
pkgCache::VerIterator CacheSetHelper::canNotGetVersion(enum VerSelector const select, pkgCacheFile &Cache, pkgCache::PkgIterator const &Pkg)
{
  ...
  case INSTALLED:
    return canNotFindInstalledVer(Cache, Pkg);
  case CANDINST:
    return canNotGetCandInstVer(Cache, Pkg);
  case INSTCAND:
    return canNotGetInstCandVer(Cache, Pkg);
  case RELEASE:
    return canNotGetVerFromRelease(Cache, Pkg, getLastVersionMatcher());
  case VERSIONNUMBER:
    return canNotGetVerFromVersionNumber(Cache, Pkg, getLastVersionMatcher());
  case ALL:
  case CANDANDINST:
                                          ...
                                          std::string const &pattern)
{
  ...
  case FNMATCH:
    ...
    break;
  case PATTERN:
    showPatternSelection(pkg, pattern);
    break;
  case PACKAGENAME: /* no surprises here */
    break;
  case STRING: /* handled by the special cases */
    break;
  case UNKNOWN:
    break;
  }
...
CacheSetHelper::CacheSetHelper(bool const ShowError, GlobalError::MsgType ErrorType) : ShowError(ShowError), ErrorType(ErrorType), d(new Private{}) {}
