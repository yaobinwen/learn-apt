...
class APT_PUBLIC pkgCache::GrpIterator : public Iterator<Group, GrpIterator>
{
  ...
  inline GrpIterator operator++(int)
  {
    GrpIterator const tmp(*this);
    operator++();
    return tmp;
  }
...
class APT_PUBLIC pkgCache::PkgIterator : public Iterator<Package, PkgIterator>
{
  ...
  inline PkgIterator operator++(int)
  {
    PkgIterator const tmp(*this);
    operator++();
    return tmp;
  }
  ...
  APT_DEPRECATED_MSG("Use APT::PrettyPkg instead")
...
class APT_PUBLIC pkgCache::VerIterator : public Iterator<Version, VerIterator>
{
  ...
  // Iteration
  inline VerIterator &operator++()
  {
    if (S != Owner->VerP)
      S = Owner->VerP + S->NextVer;
    return *this;
  }
  inline VerIterator operator++(int)
  {
    VerIterator const tmp(*this);
    operator++();
    return tmp;
  }
...
class APT_PUBLIC pkgCache::DescIterator : public Iterator<Description, DescIterator>
{
  ...
  // Iteration
  inline DescIterator &operator++()
  {
    if (S != Owner->DescP)
      S = Owner->DescP + S->NextDesc;
    return *this;
  }
  inline DescIterator operator++(int)
  {
    DescIterator const tmp(*this);
    operator++();
    return tmp;
  }
...
class APT_PUBLIC pkgCache::DepIterator : public Iterator<Dependency, DepIterator>
{
  ...
  inline DepIterator operator++(int)
  {
    DepIterator const tmp(*this);
    operator++();
    return tmp;
  }
  ...
  inline PkgIterator SmartTargetPkg() const
  {
    PkgIterator R(*Owner, 0);
    SmartTargetPkg(R);
    return R;
  }
  ...
  // Nice printable representation
  APT_DEPRECATED_MSG("Use APT::PrettyDep instead")
  friend std::ostream &operator<<(std::ostream &out, DepIterator D);

  inline DepIterator(pkgCache &Owner, Dependency *Trg, Version * = 0) : Iterator<Dependency, DepIterator>(Owner, Trg), Type(DepVer), S2(Trg == 0 ? Owner.DepDataP : (Owner.DepDataP + Trg->DependencyData))
  {
    if (S == 0)
      S = Owner.DepP;
  }
  inline DepIterator(pkgCache &Owner, Dependency *Trg, Package *) : Iterator<Dependency, DepIterator>(Owner, Trg), Type(DepRev), S2(Trg == 0 ? Owner.DepDataP : (Owner.DepDataP + Trg->DependencyData))
  {
    if (S == 0)
      S = Owner.DepP;
  }
...
class APT_PUBLIC pkgCache::PrvIterator : public Iterator<Provides, PrvIterator>
{
  ...
  inline PrvIterator operator++(int)
  {
    PrvIterator const tmp(*this);
    operator++();
    return tmp;
  }
  ...
  inline PrvIterator(pkgCache &Owner, Provides *Trg, Version *) : Iterator<Provides, PrvIterator>(Owner, Trg), Type(PrvVer)
  {
    if (S == 0)
      S = Owner.ProvideP;
  }
  inline PrvIterator(pkgCache &Owner, Provides *Trg, Package *) : Iterator<Provides, PrvIterator>(Owner, Trg), Type(PrvPkg)
  {
    if (S == 0)
      S = Owner.ProvideP;
  }
...
class APT_PUBLIC pkgCache::RlsFileIterator : public Iterator<ReleaseFile, RlsFileIterator>
{
  ...
  // Iteration
  inline RlsFileIterator &operator++()
  {
    if (S != Owner->RlsFileP)
      S = Owner->RlsFileP + S->NextFile;
    return *this;
  }
  inline RlsFileIterator operator++(int)
  {
    RlsFileIterator const tmp(*this);
    operator++();
    return tmp;
  }
...
class APT_PUBLIC pkgCache::PkgFileIterator : public Iterator<PackageFile, PkgFileIterator>
{
  ...
  // Iteration
  inline PkgFileIterator &operator++()
  {
    if (S != Owner->PkgFileP)
      S = Owner->PkgFileP + S->NextFile;
    return *this;
  }
  inline PkgFileIterator operator++(int)
  {
    PkgFileIterator const tmp(*this);
    operator++();
    return tmp;
  }
...
class APT_PUBLIC pkgCache::VerFileIterator : public pkgCache::Iterator<VerFile, VerFileIterator>
{
  ...
  // Iteration
  inline VerFileIterator &operator++()
  {
    if (S != Owner->VerFileP)
      S = Owner->VerFileP + S->NextFile;
    return *this;
  }
  inline VerFileIterator operator++(int)
  {
    VerFileIterator const tmp(*this);
    operator++();
    return tmp;
  }
...
class APT_PUBLIC pkgCache::DescFileIterator : public Iterator<DescFile, DescFileIterator>
{
  ...
  // Iteration
  inline DescFileIterator &operator++()
  {
    if (S != Owner->DescFileP)
      S = Owner->DescFileP + S->NextFile;
    return *this;
  }
  inline DescFileIterator operator++(int)
  {
    DescFileIterator const tmp(*this);
    operator++();
    return tmp;
  }
