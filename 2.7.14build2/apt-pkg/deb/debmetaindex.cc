...
static std::string NormalizeSignedBy(std::string SignedBy, bool const Introducer)
{
  ...
  auto const isAnEmptyString = [](std::string const &s)
  { return s.empty(); };
...
debReleaseIndex::debReleaseIndex(std::string const &URI, std::string const &Dist, std::map<std::string, std::string> const &Options) : metaIndex(URI, Dist, "deb"), d(new debReleaseIndexPrivate(Options))
{
}
debReleaseIndex::debReleaseIndex(std::string const &URI, std::string const &Dist, bool const pTrusted, std::map<std::string, std::string> const &Options) : metaIndex(URI, Dist, "deb"), d(new debReleaseIndexPrivate(Options))
{
  ...
  std::string DefKeepCompressedAs;
  {
    ...
    if (comps.empty() == false)
    {
      std::sort(comps.begin(), comps.end(),
                [](APT::Configuration::Compressor const &a, APT::Configuration::Compressor const &b)
                                   ...
                                   std::string &Name, std::string &Hash, unsigned long long &Size)
{
  ...
  while ((*EntryEnd != '\t' && *EntryEnd != ' ') && EntryEnd < End)
    EntryEnd++;
  if (EntryEnd == End)
    return false;
