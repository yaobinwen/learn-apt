  ...
  */
  ...
  if (pkgname.find(":") == std::string::npos)
  {
    ...
    else
    {
      ...
      {
        auto const PkgHasCurrentVersion = [](pkgCache::PkgIterator const &P)
        { return P->CurrentVer != 0; };
    ...
    if (likely(Grp.end() == false))
    {
      for (auto P = Grp.PackageList(); P.end() != true; P = Grp.NextPkg(P))
        if (Cache[P].Install())
        {
          ...
          auto const unpackOp = std::find_if(Ops.cbegin(), Ops.cend(), [](DpkgState const &s)
                                             { return strcmp(s.state, "unpacked") == 0; });
...
#undef HISTORYINFO
      ...
      case CURRENT_CANDIDATE:
        line->append(Cache[I].CurVersion).append(", ").append(Cache[I].CandVersion);
        break;
      case CURRENT:
        line->append(Cache[I].CurVersion);
        break;
      }
      line->append("), ");
    }
...
bool pkgDPkgPM::Go(APT::Progress::PackageManager *progress)
{
...
#endif
           [&C](pkgCache::VerIterator const &V)
           { return V.ParentPkg() == C.first->Pkg; });
  ...
  while (I != List.end())
  {
    ...
    if (TriggersPending == true)
      ...
      J = std::find_if(J, List.cend(), [](Item const &I)
                       { return I.Op != Item::Remove && I.Op != Item::Purge; });
    else
      J = std::find_if(J, List.cend(), [&J](Item const &I)
                       { return I.Op != J->Op; });
  ...
  if (noopDPkgInvocation == false)
  {
    if (d->dpkg_error.empty() && (PackagesDone + 1) != PackagesTotal)
    {
      ...
      _error->Warning("APT had planned for dpkg to do more than it reported back (%u vs %u).\n"
                      "Affected packages: %s",
                      PackagesDone, PackagesTotal, pkglist.c_str());
    }
...
void pkgDPkgPM::WriteApportReport(const char *pkgpath, const char *errormsg)
{
  ...
  for (auto &&I : List)
  {
    ...
    case Item::Purge:
      ...
      break;
    case Item::ConfigurePending:
      opstr = "ConfigurePending";
      break;
    case Item::TriggersPending:
      opstr = "TriggersPending";
      break;
    case Item::RemovePending:
      opstr = "RemovePending";
      break;
