...
bool pkgDepCache::writeStateFile(OpProgress *const /*prog*/, bool const InstalledOnly) /*{{{*/
{
  ...
  while (tagfile.Step(section))
  {
    ...
    if (InstalledOnly && ((pkg->CurrentVer == 0 && P.Mode != ModeInstall) ||
...
void pkgDepCache::AddStates(const PkgIterator &Pkg, bool const Invert)
{
  ...
  if (State.Status == 0)
  {
    if (State.Mode == ModeDelete)
      iDelCount += Add;
    else if ((State.iFlags & ReInstall) == ReInstall)
      iInstCount += Add;
    return;
  }
...
static APT::VersionVector getAllPossibleSolutions(pkgDepCache &Cache, pkgCache::DepIterator Start, pkgCache::DepIterator const &End, APT::CacheSetHelper::VerSelector const selector, bool const sorted) /*{{{*/
{
  ...
  if (not sorted)
    std::sort(toUpgrade.begin(), toUpgrade.end(), [](pkgCache::VerIterator const &A, pkgCache::VerIterator const &B)
              { return A->ID < B->ID; });
...
{
  auto const IsSatisfiedByInstalled = [&](auto &D)
  { return (Cache[pkgCache::DepIterator{Cache, &D}] & pkgDepCache::DepInstall) == pkgDepCache::DepInstall; };
                        ...
                        std::unique_ptr<APT::CacheFilter::Matcher> &IsProtectedKernelPackage)
{
  ...
  for (auto D = Ver.DependsList(); not D.end(); ++D)
  {
    ...
    for (auto Prv = T.ProvidesList(); not Prv.end(); ++Prv)
    {
      ...
      auto const PV = (PkgState[PP->ID].Install()) ? PkgState[PP->ID].InstVerIter(DepCache) : PP.CurrentVer();
    ...
    for (auto &providers : providers_by_source)
    {
      ...
      providers.second.erase(std::remove_if(providers.second.begin(), providers.second.end(), [&](auto const &V)
                                            { return strcmp(highestSrcVer, V.SourceVerStr()) != 0; }),
      ...
      if (not std::all_of(providers.second.begin(), providers.second.end(), [&](auto const &Prv)
                          { return (*IsAVersionedKernelPackage)(Prv.ParentPkg()); }))
        continue;
      ...
      if (not std::any_of(providers.second.begin(), providers.second.end(), [&](auto const &Prv)
                          { return (*IsProtectedKernelPackage)(Prv.ParentPkg()); }))
        continue;
      providers.second.erase(std::remove_if(providers.second.begin(), providers.second.end(),
                                            [&](auto const &Prv)
                                            { return not((*IsProtectedKernelPackage)(Prv.ParentPkg())); }),
                             providers.second.end());
    }
