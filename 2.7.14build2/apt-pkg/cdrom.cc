...
bool pkgCdrom::WriteSourceList(string Name, vector<string> &List, bool Source)
{
  ...
  while (F.ReadLine(Buffer))
  {
    ...
    if (First == true)
    {
      for (vector<string>::iterator I = List.begin(); I != List.end(); ++I)
      {
        ...
        Out << Type << " cdrom:[" << Name << "]/" << string(*I, 0, Space) << " " << string(*I, Space + 1) << endl;
      }
    }
    First = false;
  ...
  if (First == true)
  {
    for (vector<string>::iterator I = List.begin(); I != List.end(); ++I)
    {
      ...
      Out << "deb cdrom:[" << Name << "]/" << string(*I, 0, Space) << " " << string(*I, Space + 1) << endl;
    }
  }
...
bool pkgCdrom::Add(pkgCdromStatus *log) /*{{{*/
{
  ...
  for (vector<string>::iterator I = List.begin(); I != List.end(); ++I)
  {
    ...
    {
      stringstream msg;
      msg << "deb cdrom:[" << Name << "]/" << string(*I, 0, Space) << " " << string(*I, Space + 1) << endl;
      log->Update(msg.str());
    }
  }
  ...
  for (vector<string>::iterator I = SourceList.begin(); I != SourceList.end(); ++I)
  {
    ...
    {
      stringstream msg;
      msg << "deb-src cdrom:[" << Name << "]/" << string(*I, 0, Space) << " " << string(*I, Space + 1) << endl;
      log->Update(msg.str());
    }
  }
