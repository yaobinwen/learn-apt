...
static bool WriteOkay_fn(FileFd &output, APT::StringView data, Tail... more_data)
{
  return likely(output.Write(data.data(), data.length()) && WriteOkay_fn(output, more_data...));
}
template <typename... Tail>
static bool WriteOkay_fn(FileFd &output, unsigned int data, Tail... more_data)
{
  std::string number;
...
template <typename... Data>
static bool WriteOkay(FileFd &output, Data &&...data)
{
