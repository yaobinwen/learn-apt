...
static bool TargetIsAllowedToBe(IndexTarget const &Target, InsecureType const type)
{
  ...
  switch (type)
  {
  case InsecureType::UNSIGNED:
    break;
  case InsecureType::NORELEASE:
    break;
                                                           ...
                                                           metaIndex const *const MetaIndexParser, pkgAcqMetaClearSig *const TransactionManager, pkgAcquire::Item *const I)
{
  ...
  if (msg != InsecureType::WEAK)
  {
    ...
    if (RealFileExists(FinalReleasegpg) || RealFileExists(FinalInRelease))
    {
      ...
      {
      case InsecureType::UNSIGNED:
        msgstr = _("The repository '%s' is no longer signed.");
        break;
      case InsecureType::NORELEASE:
        msgstr = _("The repository '%s' no longer has a Release file.");
        break;
      case InsecureType::WEAK: /* unreachable */
        break;
      }
  ...
  {
  case InsecureType::UNSIGNED:
    msgstr = _("The repository '%s' is not signed.");
    break;
  case InsecureType::NORELEASE:
    msgstr = _("The repository '%s' does not have a Release file.");
    break;
  case InsecureType::WEAK:
    msgstr = _("The repository '%s' provides only weak security information.");
    break;
  }
...
bool pkgAcqTransactionItem::TransactionState(TransactionStates const state)
{
  ...
  case TransactionStarted:
    _error->Fatal("Item %s changed to invalid transaction start state!", Target.URI.c_str());
    break;
...
bool pkgAcqIndex::TransactionState(TransactionStates const state)
{
  ...
  {
  case TransactionStarted:
    _error->Fatal("AcqIndex %s changed to invalid transaction start state!", Target.URI.c_str());
    break;
  case TransactionAbort:
...
bool pkgAcqDiffIndex::TransactionState(TransactionStates const state)
{
  ...
  {
  case TransactionStarted:
    _error->Fatal("Item %s changed to invalid transaction start state!", GetDiffIndexURI(Target).c_str());
    break;
  case TransactionCommit:
   ...
   Handy in cases in which we know we have the file already, like IMS-Hits. */
{
  ...
  NoActionItem(pkgAcquire *const Owner, IndexTarget const &Target) : pkgAcquire::Item(Owner), Target(Target)
  {
    Status = StatDone;
    DestFile = GetFinalFileNameFromURI(Target.URI);
  }
  NoActionItem(pkgAcquire *const Owner, IndexTarget const &Target, std::string const &FinalFile) : pkgAcquire::Item(Owner), Target(Target)
  {
    Status = StatDone;
    DestFile = FinalFile;
  }
   ...
   even if the lists-cleanup is deactivated. */
{
  ...
  CleanupItem(pkgAcquire *const Owner, pkgAcqMetaClearSig *const TransactionManager, IndexTarget const &Target) : pkgAcqTransactionItem(Owner, TransactionManager, Target)
  {
    Status = StatDone;
    DestFile = GetFinalFileNameFromURI(Target.URI);
  }
...
pkgAcquire::Item::Item(pkgAcquire *const owner) : FileSize(0), PartialSize(0), ID(0), Complete(false), Local(false),
...
int pkgAcquire::Item::Priority() /*{{{*/
{
  ...
  //   for the rest
  if (dynamic_cast<pkgAcqMetaSig *>(this) != nullptr || dynamic_cast<pkgAcqMetaBase *>(this) != nullptr || dynamic_cast<pkgAcqDiffIndex *>(this) != nullptr)
    return 1000;
  // Stage 2: Diff files
...
pkgAcqTransactionItem::pkgAcqTransactionItem(pkgAcquire *const Owner, /*{{{*/
                                             pkgAcqMetaClearSig *const transactionManager, IndexTarget const &target) : pkgAcquire::Item(Owner), d(NULL), Target(target), TransactionManager(transactionManager)
{
...
void pkgAcqMetaBase::AbortTransaction()
{
  ...
  case TransactionStarted:
    break;
  case TransactionAbort:
    _error->Fatal("Transaction %s was already aborted and is aborted again", TransactionManager->Target.URI.c_str());
    return;
  case TransactionCommit:
    _error->Fatal("Transaction %s was already aborted and is now committed", TransactionManager->Target.URI.c_str());
    return;
  }
...
void pkgAcqMetaBase::CommitTransaction()
{
  ...
  case TransactionStarted:
    break;
  case TransactionAbort:
    _error->Fatal("Transaction %s was already committed and is now aborted", TransactionManager->Target.URI.c_str());
    return;
  case TransactionCommit:
    _error->Fatal("Transaction %s was already committed and is again committed", TransactionManager->Target.URI.c_str());
    return;
  }
...
void pkgAcqMetaClearSig::QueueIndexes(bool const verify) /*{{{*/
{
  ...
  if (hasReleaseFile && verify == false)
    hasHashes = std::any_of(IndexTargets.begin(), IndexTargets.end(),
                            [&](IndexTarget const &Target)
                            { return TransactionManager->MetaIndexParser->Exists(Target.MetaKey); });
   ...
   which is native/foreign arch, specific order of preference of translations, … */
    ...
    do
    {
      ...
      auto const range_end = std::find_if_not(range_start, IndexTargets.end(),
                                              [&type](IndexTarget const &T)
                                              { return type == T.Option(IndexTarget::CREATED_BY); });
...
bool pkgAcqMetaBase::VerifyVendor(string const &) /*{{{*/
{
  ...
  if (TransactionManager->LastMetaIndexParser != nullptr)
  {
    ...
    if (Changes.empty() == false)
    {
      ...
      if (std::any_of(Changes.begin(), Changes.end(), [](pkgAcquireStatus::ReleaseInfoChange const &c)
                      { return c.DefaultAction == false; }))
      {
        std::string msg;
        // TRANSLATOR: %s is the name of the manpage in question, e.g. apt-secure(8)
        strprintf(msg, _("This must be accepted explicitly before updates for "
                         "this repository can be applied. See %s manpage for details."),
                  "apt-secure(8)");
...
pkgAcqMetaClearSig::pkgAcqMetaClearSig(pkgAcquire *const Owner, /*{{{*/
                                       ...
                                       metaIndex *const MetaIndexParser) : pkgAcqMetaIndex(Owner, this, ClearsignedTarget, DetachedSigTarget),
...
pkgAcqMetaIndex::pkgAcqMetaIndex(pkgAcquire *const Owner, /*{{{*/
                                 pkgAcqMetaClearSig *const TransactionManager,
                                 IndexTarget const &DataTarget,
                                 IndexTarget const &DetachedSigTarget) : pkgAcqMetaBase(Owner, TransactionManager, DataTarget), d(NULL),
...
pkgAcqMetaSig::pkgAcqMetaSig(pkgAcquire *const Owner,
                             pkgAcqMetaClearSig *const TransactionManager,
                             IndexTarget const &Target,
                             pkgAcqMetaIndex *const MetaIndex) : pkgAcqTransactionItem(Owner, TransactionManager, Target), d(NULL), MetaIndex(MetaIndex)
{
...
bool pkgAcqDiffIndex::ParseDiffIndex(string const &IndexDiffFile) /*{{{*/
{
...

  {
    auto const foundStart = std::find_if(available_patches.rbegin(), available_patches.rend(),
                                         [&](auto const &cur)
                                         { return LocalHashes == cur.result_hashes; });
                                 ...
                                 pkgAcquire::MethodConfig const *const Cnf)
{
  ...
  if (std::any_of(allPatches->begin(), allPatches->end(),
                  [](pkgAcqIndexMergeDiffs const *const P)
                  { return P->State == StateErrorDiff; }))
  {
    if (Debug)
  ...
  case StateApplyDiff:
    ...
    return;
  case StateDoneDiff:
    _error->Fatal("Done called for %s which is in an invalid Done state", patch.file.c_str());
    break;
  case StateErrorDiff:
    _error->Fatal("Done called for %s which is in an invalid Error state", patch.file.c_str());
    break;
  }
}
/*}}}*/
...
pkgAcqChangelog::pkgAcqChangelog(pkgAcquire *const Owner, pkgCache::VerIterator const &Ver,
                                 std::string const &DestDir, std::string const &DestFilename) : pkgAcquire::Item(Owner), d(new pkgAcqChangelog::Private()), SrcName(Ver.SourcePkgName()), SrcVersion(Ver.SourceVerStr())
{
  Desc.URI = URI(Ver);
...
pkgAcqChangelog::pkgAcqChangelog(pkgAcquire *const Owner, pkgCache::RlsFileIterator const &RlsFile,
                                 ...
                                 const string &DestDir, const string &DestFilename) : pkgAcquire::Item(Owner), d(new pkgAcqChangelog::Private()), SrcName(SrcName), SrcVersion(SrcVersion)
{
        ...
        if (trimmed.IsOpen())
        {
         ...
         avoid doing this costly dance on at least a bit of the file. */
          ...
          while (trimmed.ReadLine(buffer, sizeof(buffer)) != nullptr)
          {
            ...
            if (std::any_of(std::begin(giveaways), std::end(giveaways), [=](auto const gw)
                            { return line.compare(0, gw.size(), gw) == 0; }))
            {
              trimmedFile = true;
              break;
            }
