...
APT_DEPRECATED_MSG("Use SubstVar to avoid memory headaches")
...
APT_PURE APT_HOT static inline int tolower_ascii_unsafe(int const c)
{
  return c | 0x20;
}
APT_PURE APT_HOT static inline int tolower_ascii_inline(int const c)
{
  return (c >= 'A' && c <= 'Z') ? c + 32 : c;
}
APT_PURE APT_HOT static inline int isspace_ascii_inline(int const c)
{
  // 9='\t',10='\n',11='\v',12='\f',13='\r',32=' '
  return (c >= 9 && c <= 13) || c == ' ';
}
APT_PURE APT_HOT static inline int islower_ascii(int const c)
{
  return c >= 'a' && c <= 'z';
}
APT_PURE APT_HOT static inline int isupper_ascii(int const c)
{
  return c >= 'A' && c <= 'Z';
}
APT_PURE APT_HOT static inline int isalpha_ascii(int const c)
{
