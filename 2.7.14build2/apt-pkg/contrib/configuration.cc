...
enum class APT_HIDDEN ConfigType
{
  UNDEFINED,
  INT,
  BOOL,
...
static std::string getConfigTypeString(ConfigType const type) /*{{{*/
{
  ...
  case ConfigType::STRING:
    return "STRING";
  case ConfigType::STRING_OR_BOOL:
  ...
  case ConfigType::LIST:
    return "LIST";
  case ConfigType::STRING_OR_LIST:
                    ...
                    unsigned const &Depth)
{
  ...
  while (F.Eof() == false && F.Failed() == false)
  {
    ...
    // Discard single line comments
    {
      ...
      {
        if (std::count(Line.begin(), Line.begin() + start, '"') % 2 != 0 ||
            std::any_of(magicComments.begin(), magicComments.end(), [&](auto const m)
                        { return Line.compare(start + 1, m.length(), m) == 0; }))
        {
          ++start;
          continue;
        }
