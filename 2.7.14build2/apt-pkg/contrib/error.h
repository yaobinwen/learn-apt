...
class APT_PUBLIC GlobalError /*{{{*/
{
  ...
  struct Item
  {
    ...
    Item(char const *Text, MsgType const &Type) : Text(Text), Type(Type) {};
  ...
  struct MsgStack
  {
    ...
    MsgStack(std::list<Item> const &Messages, bool const &Pending) : Messages(Messages), PendingFlag(Pending) {};
  };
