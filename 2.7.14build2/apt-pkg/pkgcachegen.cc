...
template <class T>
using Dynamic = pkgCacheGenerator::Dynamic<T>;
typedef std::vector<pkgIndexFile *>::iterator FileIterator;
template <typename Iter>
std::vector<Iter *> pkgCacheGenerator::Dynamic<Iter>::toReMap;
...
pkgCacheGenerator::pkgCacheGenerator(DynamicMMap *pMap, OpProgress *Prog) : Map(*pMap), Cache(pMap, false), Progress(Prog),
...
#undef APT_REMAP
  ...
  for (APT::StringView *ViewP : Dynamic<APT::StringView>::toReMap)
  {
    ...
    if (ViewP->data() < static_cast<const char *>(oldMap) || ViewP->data() > static_cast<const char *>(oldMap) + oldSize)
      continue;
                                     ...
                                     uint8_t const Flags)
{
  ...
  if (Ver.ParentPkg().Name() == PkgName && (PkgArch == Ver.ParentPkg().Arch() || (PkgArch == "all" && strcmp((Cache.StrP + Cache.HeaderP->Architecture), Ver.ParentPkg().Arch()) == 0)) &&
                                                ...
                                                unsigned int Size)
{
  ...
  case SECTION:
    ...
    break;
  default:
    _error->Fatal("Unknown enum type used for string storage of '%.*s'", Size, S);
    return 0;
  }
                          ...
                          pkgCache **OutCache = 0)
{
  ...
  for (pkgSourceList::const_iterator i = List.begin(); i != List.end(); ++i)
  {
    ...
    std::copy_if(Indexes->begin(), Indexes->end(), std::back_inserter(Files),
                 [](pkgIndexFile const *const I)
                 { return I->HasPackages(); });
  }
