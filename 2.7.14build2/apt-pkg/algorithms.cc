...
void pkgProblemResolver::MakeScores()
{
  ...
  for (pkgCache::PkgIterator I = Cache.PkgBegin(); I.end() == false; ++I)
  {
   ...
   a conflicts on a powerful normal package (ie libc6) */
    if ((I->Flags & pkgCache::Flag::Essential) == pkgCache::Flag::Essential || (I->Flags & pkgCache::Flag::Important) == pkgCache::Flag::Important)
...
bool pkgProblemResolver::ResolveInternal(bool const BrokenFix)
{
  ...
  std::sort(PList.get(), PEnd, [this](Package *a, Package *b)
            { return ScoreSort(a, b) < 0; });
    ...
    for (pkgCache::Package **K = PList.get(); K != PEnd; K++)
    {
           ...
           D.end() == false || InOr == true;)
      {
        ...
        for (pkgCache::Version **V = VList.get(); *V != 0; V++)
        {
          ...
          if (Debug == true)
            clog << "  Considering " << Pkg.FullName(false) << ' ' << Scores[Pkg->ID] << " as a solution to " << I.FullName(false) << ' ' << Scores[I->ID] << endl;
...
bool pkgProblemResolver::ResolveByKeepInternal()
{
  ...
  std::sort(PList, PEnd, [this](Package *a, Package *b)
            { return ScoreSort(a, b) < 0; });
