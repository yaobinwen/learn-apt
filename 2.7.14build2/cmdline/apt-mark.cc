...
static bool DoMinimize(CommandLine &CmdL)
{
  ...
  ShowList(c1out, _("The following packages will be marked as automatically installed:"), changed, [](const pkgCache::PkgIterator &)
           { return true; }, &PrettyFullName, &PrettyFullName);
...
static bool DoSelection(CommandLine &CmdL)
{
  ...
  if (strcasecmp(CmdL.FileList[0], "hold") == 0 || strcasecmp(CmdL.FileList[0], "unhold") == 0)
  {
    auto const part = std::stable_partition(pkgset.begin(), pkgset.end(),
                                            ...
                                            { return V.ParentPkg()->SelectedState == pkgCache::State::Hold; });
...
{
  std::cout << _("Usage: apt-mark [options] {auto|manual} pkg1 [pkg2 ...]\n"
                 "\n"
...
static std::vector<aptDispatchWithHelp> GetCommands() /*{{{*/
{
  return {
     ...
     {"showhold", &ShowSelection, _("Print the list of packages on hold")},
     {"showholds", &ShowSelection, nullptr},
     {"showheld", &ShowSelection, nullptr},
     {"showinstall", &ShowSelection, nullptr},
     {"showinstalls", &ShowSelection, nullptr},
     {"showdeinstall", &ShowSelection, nullptr},
     {"showdeinstalls", &ShowSelection, nullptr},
     {"showremove", &ShowSelection, nullptr},
     {"showremoves", &ShowSelection, nullptr},
     {"showpurge", &ShowSelection, nullptr},
     {"showpurges", &ShowSelection, nullptr},
