                                    ...
                                    SignersStorage &Signers)
{
  ...
  while (1)
  {
    ...
    else if (strncmp(buffer, GNUPGNOPUBKEY, sizeof(GNUPGNOPUBKEY) - 1) == 0)
    {
      ...
      ErrSigners.erase(std::remove_if(ErrSigners.begin(), ErrSigners.end(), [&](std::string const &errsig)
                                      { return errsig.compare(strlen("ERRSIG "), 16, buffer, sizeof(GNUPGNOPUBKEY), 16) == 0; }),
                       ErrSigners.end());
    }
  ...
  else
  {
    ...
    for (auto const &v : Signers.Valid)
      if (std::any_of(Signers.Good.begin(), Signers.Good.end(),
                      [&v](std::string const &g)
                      { return IsTheSameKey(v, g); }))
      ...
      if (std::any_of(sub.second.begin(), sub.second.end(),
                      [&](std::string const &s)
                      {
                        ...
                        return std::any_of(Signers.Good.begin(), Signers.Good.end(),
                                           [&s](std::string const &g)
                                           { return IsTheSameKey(s, g); });
                      }))
  ...
  if (Debug)
  {
    ...
    std::cerr << "\n  Worthless: ";
    std::for_each(Signers.Worthless.begin(), Signers.Worthless.end(), [](Signer const &sig)
                  { std::cerr << sig.key << ", "; });
    std::cerr << "\n  SoonWorthless: ";
    std::for_each(Signers.SoonWorthless.begin(), Signers.SoonWorthless.end(), [](Signer const &sig)
                  { std::cerr << sig.key << ", "; });
    std::cerr << "\n  NoPubKey: ";
    ...
    std::cerr << std::endl
              << "  NODATA: " << (gotNODATA ? "yes" : "no") << std::endl;
  }
