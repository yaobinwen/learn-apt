...
bool CheckNothingBroken(CacheFile &Cache) /*{{{*/
{
  ...
  c1out << _("Some packages could not be installed. This may mean that you have\n"
...
bool InstallPackages(CacheFile &Cache, APT::PackageVector &HeldBackPackages, bool ShwKept, bool Ask, bool Safety, std::string const &Hook, CommandLine const &CmdL)
{
  ...
  if (_config->FindB("APT::Get::Print-URIs") == true)
  {
    ...
    for (; I != Fetcher.UriEnd(); ++I)
      std::cout << '\'' << I->URI << "' " << flNotDir(I->Owner->DestFile) << ' ' << std::to_string(I->Owner->FileSize) << ' ' << I->Owner->HashSum() << std::endl;
    return true;
  }
  ...
  if (disappearedPkgs.empty() == false)
  {
    ShowList(c1out, P_("The following package disappeared from your system as\n"
                       ...
                       "all files have been overwritten by other packages:",
                       disappearedPkgs.size()),
             disappearedPkgs, [](std::string const &Pkg)
             { return Pkg.empty() == false; }, [](std::string const &Pkg)
             { return Pkg; }, [](std::string const &)
             { return std::string(); });
...
bool DoAutomaticRemove(CacheFile &Cache)
{
  ...
  if (doAutoRemove == false && autoRemoveCount != 0)
  {
    if (smallList == false)
    {
      ...
      Cache->MarkAndSweep();
      SortedPackageUniverse Universe(Cache);
      ShowList(c1out, P_("The following package was automatically installed and is no longer required:", "The following packages were automatically installed and are no longer required:", autoRemoveCount), Universe, [&Cache](pkgCache::PkgIterator const &Pkg)
               { return (*Cache)[Pkg].Garbage == true && (*Cache)[Pkg].Delete() == false; }, &PrettyFullName, CandidateVersion(&Cache));
    }
    else
                                        ...
                                        std::set<std::string> &UnknownPackages, APT::PackageVector &HeldBackPackages)
{
  ...
  for (auto &vs : verset)
  {
    ...
    vs.second.erase(std::remove_if(vs.second.begin(), vs.second.end(), [&](auto const &p)
                                   { return not seen.insert(p->ID).second; }),
                    vs.second.end());
  }
...
bool AddVolatileSourceFile(pkgSourceList *const SL, PseudoPkg &&pkg, std::vector<PseudoPkg> &VolatileCmdL) /*{{{*/
{
  ...
  std::transform(files.begin(), files.end(), std::back_inserter(VolatileCmdL), [&](auto &&f)
                 { return PseudoPkg{std::move(f), pkg.arch, pkg.release, pkg.index}; });
  return true;
}
/*}}}*/
...
bool DoInstall(CommandLine &CmdL)
{
  ...
  /* Print out a list of suggested and recommended packages */
  {
        ...
        {
          {
            ...
            if (std::find_if(installed.begin(), installed.end(),
                             [&Cache](pkgCache::VerIterator const &Ver)
                             { return Cache[Ver.ParentPkg()].Delete() == false; }) != installed.end())
            {
...

          {
            ...
            if (std::find_if(upgrades.begin(), upgrades.end(),
                             [&Cache](pkgCache::VerIterator const &Ver)
                             { return Cache[Ver.ParentPkg()].Upgrade(); }) != upgrades.end())
            {
              foundInstalledInOrGroup = true;
              break;
            }
    ...
    auto always_true = [](std::string const &)
    { return true; };
    auto string_ident = [](std::string const &str)
    { return str; };
