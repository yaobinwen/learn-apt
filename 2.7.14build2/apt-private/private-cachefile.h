...
class SortedPackageUniverse : public APT::PackageUniverse
{
  ...
  class const_iterator : public APT::Container_iterator_base<APT::PackageContainerInterface, SortedPackageUniverse, SortedPackageUniverse::const_iterator, std::vector<map_pointer<pkgCache::Package>>::const_iterator, pkgCache::PkgIterator>
  {
    ...
    {
      if (*_iter == 0)
        return pkgCache::PkgIterator(*Cache);
      return pkgCache::PkgIterator(*Cache, Cache->PkgP + *_iter);
    }
    explicit const_iterator(pkgCache *const Owner, std::vector<map_pointer<pkgCache::Package>>::const_iterator i) : Container_iterator_base<APT::PackageContainerInterface, SortedPackageUniverse, SortedPackageUniverse::const_iterator, std::vector<map_pointer<pkgCache::Package>>::const_iterator, pkgCache::PkgIterator>(i), Cache(Owner) {}
  };
  ...
  {
    LazyInit();
    return const_iterator(data(), List.begin());
  }
  const_iterator end() const
  {
    LazyInit();
    return const_iterator(data(), List.end());
  }
