...
{
  ShowList(c2out, _("WARNING: The following packages cannot be authenticated!"), UntrustedList, [](std::string const &)
           { return true; }, [](std::string const &str)
           { return str; }, [](std::string const &)
           { return ""; });
...
bool DoDownload(CommandLine &CmdL)
{
  ...
  if (_config->FindB("APT::Get::Print-URIs") == true)
  {
    ...
    for (; I != Fetcher.UriEnd(); ++I)
      std::cout << '\'' << I->URI << "' " << flNotDir(I->Owner->DestFile) << ' ' << I->Owner->FileSize << ' ' << I->Owner->HashSum() << std::endl;
    return true;
  }
