...
CacheSetHelperVirtuals::CacheSetHelperVirtuals(bool const ShowErrors, GlobalError::MsgType const &ErrorType) : CacheSetHelper{ShowErrors, ErrorType}
{
}
/*}}}*/

// CacheSetHelperAPTGet - responsible for message telling from the CacheSets/*{{{*/
CacheSetHelperAPTGet::CacheSetHelperAPTGet(std::ostream &pout) : APT::CacheSetHelper{true}, out(pout)
{
                                                        ...
                                                        CacheSetHelper::VerSelector const select)
{
  ...
  if (select == VERSIONNUMBER || select == RELEASE)
    for (auto const &V : verset)
      if (std::count_if(verset.begin(), verset.end(), [Pkg = V.ParentPkg()](auto const &v)
                        { return v.ParentPkg() == Pkg; }) == 1)
