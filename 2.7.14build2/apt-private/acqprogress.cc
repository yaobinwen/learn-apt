...
// ---------------------------------------------------------------------
/* */
AcqTextStatus::AcqTextStatus(std::ostream &out, unsigned int &ScreenWidth, unsigned int const Quiet) : pkgAcquireStatus(), out(out), ScreenWidth(ScreenWidth), LastLineLength(0), ID(0), Quiet(Quiet)
{
...
void AcqTextStatus::Fail(pkgAcquire::ItemDesc &Itm)
{
  ...
  if (ShowErrorText)
  {
    ...
    {
      out << std::endl
          << "  " << Itm.Owner->ErrorText.substr(line_start, line_end - line_start);
    ...
    if (line_start == 0)
      ...
      out << std::endl
          << "  " << Itm.Owner->ErrorText.substr(line_start);
  }
  out << std::endl;
